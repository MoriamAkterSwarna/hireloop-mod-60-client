            ...
            {jobs.length === 0 ? (
              ...
              jobs.map((job) => {
                const datePosted = job.createdAt
                  ...
                  : new Date(
                      parseInt(job._id.substring(0, 8), 16) * 1000,
                ...
                const companyName =
                  job.companyName || job.company?.companyName || "Unknown";
                const initial = companyName.charAt(0).toUpperCase();

                return (
                  <tr
                    key={job._id}
                    className="hover:bg-[#1f1f22] transition-colors group"
                  >
                    ...
                    <td className="py-4 px-6">
                      {(() => {
                        ...
                        if (isPastDeadline && job.status === "active") {
                            return (
                        ...
                        switch (job.status) {
                            ...
                            case 'pending':
                                return (
                                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-amber-950/30 text-amber-500 border border-amber-900/50">
                    ...
                    <td className="py-4 px-6 text-right">
                      <JobActionButtons jobId={job._id} currentStatus={job.status} />
                    </td>
                  </tr>
                );
              })
